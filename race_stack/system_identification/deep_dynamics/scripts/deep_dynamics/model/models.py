...
class DatasetBase(torch.utils.data.Dataset):
    def __init__(self, features, labels, scaler=None):
        ...
        # scaler가 None이면 StandardScaler로 정규화
        if scaler is None:
        ...
        # scaler가 None이 아니면 인자로 받은 scaler로 정규화
        else:
            self.scaler = scaler
...
class DeepDynamicsDataset(DatasetBase):
    def __init__(self, features, labels, scalers=None):
        ...
        super().__init__(features[:,:,:5], labels, scalers)
        ...
        for i in range(len(self.feed_forward)):
            if i == 0:
                if isinstance(self.feed_forward[i], torch.nn.RNNBase):
                    ff, h0 = self.feed_forward[0](x_norm, h0) # 첫 번째 레이어에서 정규화된 데이터를 사용
                    self.ff = ff
                else:
...
class DeepDynamicsModelF110(ModelBase):
    def __init__(self, param_dict, eval=False):
    ...
    def differential_equation(self, x, output, Ts=0.05):
        ...
        sys_param_dict, _ = self.unpack_sys_params(output)
        s = self.unpack_state_actions(x)  # s["VX"], s["VY"], s["YAW_RATE"], s["THROTTLE"], s["STEERING"]
        ...
        alphaf = -torch.atan2(vy + r*lf, vx) + delta
        alphar = -torch.atan2(vy - r*lr, vx)

        # Pacejka lateral forces with normal load scaling (front & rear)
        Bf, Cf, Df, Ef = sys_param_dict["Bf"], sys_param_dict["Cf"], sys_param_dict["Df"], sys_param_dict["Ef"]
        Br, Cr, Dr, Er = sys_param_dict["Br"], sys_param_dict["Cr"], sys_param_dict["Dr"], sys_param_dict["Er"]

        def pacejkaFy(Fz, B, C, D, E, alpha):
            tan_term = torch.atan(B * alpha)
            inner = B * alpha - E * (B * alpha - tan_term)
            return Fz * D * torch.sin(C * torch.atan(inner))

        Ffy = pacejkaFy(F_zf, Bf, Cf, Df, Ef, alphaf)
        Fry = pacejkaFy(F_zr, Br, Cr, Dr, Er, alphar)

        # Longitudinal force at rear 
        Cm1, Cm2, Cr0, Cr2 = sys_param_dict["Cm1"], sys_param_dict["Cm2"], sys_param_dict["Cr0"], sys_param_dict["Cr2"]
        Frx = (Cm1-Cm2*vx)*D - Cr0 - Cr2*(vx**2)

        # Dynamics (same structure as C++ getF)
        Iz = sys_param_dict["Iz"]

        dvxdt = (1.0/m) * (Frx - Ffy*torch.sin(delta) + m*vy*r)
        dvydt = (1.0/m) * (Fry + Ffy*torch.cos(delta) - m*vx*r)
        drdt  = (1.0/Iz) * (Ffy*lf*torch.cos(delta) - Fry*lr)

        dxdt = torch.stack([dvxdt, dvydt, drdt], dim=-1) * Ts
        return x[:, -1, :3] + dxdt
...
string_to_model = {
    ...
    "DeepDynamicsF110" : DeepDynamicsModelF110
}
...
string_to_dataset = {
    ...
    "DeepDynamicsF110" : DeepDynamicsDataset
}